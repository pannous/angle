...
// import {*} from 'english_tokens';
// import * as ast_magic from 'ast_magic';
// import * as _ast from '_ast';
// import {kast} from 'kast';
// import {name, store, assign, call, num, zero, Self} from 'kast/kast';
// import {*} from '_ast';
// import {operator_equals} from 'tree';
...
function statement(doraise = true) {
    ...
    maybe_indent();
    x = ((((((((((((((((maybe(quick_expression) || maybe(setter)) || maybe(returns)) || maybe(imports)) || maybe(method_definition)) || maybe(assert_that)) || maybe(breaks)) || maybe(loops)) || maybe(if_then_else)) || maybe(once)) || maybe(piped_actions)) || maybe(declaration)) || maybe(nth_item)) || maybe(neu)) || maybe(action)) || maybe(expression)) || raise_not_matching(("Not a statement %s" % pointer_string())));
...
function method_call(obj = null) {
    ...
    if (((module || obj) || is_object_method(method_name))) {
        ...
        maybe_token("of");
        obj = (maybe(list(the.classes.keys())) || maybe(the.moduleNames));
        if ((!context.in_args)) {
            obj = (obj || maybe(liste));
        }
        maybe_token(",");
...
function end_of_statement() {
    return ((((((beginning_of_line() || maybe_newline()) || starts_with(done_words)) || (the.current_offset === 0)) || (the.previous_word === ";")) || (the.previous_word === "\n")) || token(";", "end_of_statement"));
}
